...
func TestMain(m *testing.M) {
	...
	//  - 监听地址 (addr)：默认为 ":8088"；
	//  - 回调路径 (path)：默认为 "/callback"；
	//  - 日志打印器 (logger)：默认与 JSMS API v1 使用相同的日志打印器；
	//  - HTTP Handler (handler)：默认为使用 net/http 实现的一个简单的 Handler；
	//  - 「用户回复消息」SMS_REPLY 回执数据回调处理器，为 nil 时不处理；
	//  - 「短信送达状态」SMS_REPORT 回执数据回调处理器，为 nil 时不处理；
	//  - 「模板审核结果」SMS_TEMPLATE 回执数据回调处理器，为 nil 时不处理；
	//  - 「签名审核结果」SMS_SIGN 回执数据回调处理器，为 nil 时不处理。
