...
func (b *APIv3Builder) Build() (APIv3, error) {
	...
	filev3, _ := file.NewAPIv3Builder().
		...
		SetHost(b.host).
		SetAuthKey(b.appKey).
		SetAuthSecret(b.masterSecret).
		SetLogger(b.logger).
		SetHttpLogLevel(b.httpLogLevel).
		Build()

	imagev3, _ := image.NewAPIv3Builder().
		SetClient(b.client).
		SetHost(b.host).
		SetAppKey(b.appKey).
		SetMasterSecret(b.masterSecret).
		SetLogger(b.logger).
		SetHttpLogLevel(b.httpLogLevel).
		Build()

	schedulev3, _ := schedule.NewAPIv3Builder().
		SetClient(b.client).
		SetHost(b.host).
		SetAppKey(b.appKey).
		SetMasterSecret(b.masterSecret).
...
// apiv3 内部实现了 APIv3，是 Push API v3 的默认访问客户端。
type apiv3 struct {
	fileAPIv3
	imageAPIv3
	scheduleAPIv3
