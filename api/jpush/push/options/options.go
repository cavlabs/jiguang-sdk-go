...
type Options struct {
	...
	//  - 自定义事件：需集成极光分析 SDK，开发者在极光分析产品中自行创建的业务事件（如：加入购物车、浏览商品等），详情参考【如何创建自定义事件】和【SDK 如何上报自定义事件】：https://docs.jiguang.cn/public_service/dataCenter/metadata/metaEvent、https://docs.jiguang.cn/public_service/client/Android/sdk_api#%E4%B8%8A%E6%8A%A5%E8%87%AA%E5%AE%9A%E4%B9%89%E4%BA%8B%E4%BB%B6；
	...
	// 【可选】推送计划标识。
	//  - 需先通过极光 WebPortal 创建计划标识值，创建步骤参考【推送计划文档】：https://docs.jiguang.cn/jpush/console/config_manage/push_plan。
...
type ThirdPartyChannelOptions struct {
	...
	// 【可选】通知栏消息分类。
	//  - 为了适配 小米、华为、OPPO 手机厂商通知栏消息分类，由开发者自行向手机厂商申请，具体申请规则参考【厂商消息分类使用指南】：https://docs.jiguang.cn/jpush/client/Android/android_channel_id；
	//  - 注意华为数据处理位置为中国区的应用不支持该字段，详情参见【华为自定义通知渠道】：https://developer.huawei.com/consumer/cn/doc/development/HMSCore-Guides/android-custom-chan-0000001050040122；
	//  - Android 下也有 ChannelID 字段，若本字段有填充，则优先使用，若无填充则以 Android 的 ChannelID 的定义为准；
	//  - 特别注意：由于 OPPO 厂商 2024.11.20 实施【OPPO 消息分类新规】，建议您同时填写此字段和 Category, NotifyLevel 字段：https://open.oppomobile.com/new/developmentDoc/info?id=13189。
	...
	//  - 0：代表运营消息；
	//  - 1：代表系统消息。
	// 目前 vivo 对系统消息分类较为严格，具体规则参考：https://dev.vivo.com.cn/documentCenter/doc/359。
	//
	// 关于 Classification 和 SkipQuota 字段说明：
	//  - 不传递 Classification 字段，但传递 SkipQuota 时，应用是否扣除配额以客户传递的 SkipQuota 为准，需开发者自己管理配额；
	//  - 传递 Classification 时，会忽略 SkipQuota 值，极光会按照【厂商系统消息、运营消息分类】规则自动判断是否扣除配额，帮助开发者管理配额：https://docs.jiguang.cn/jpush/client/Android/android_channel_id。
	Classification *int `json:"classification,omitempty"`
	// 【可选】通知栏消息类型。
	//
	// 对应 vivo 的 pushMode 字段，不填默认为 0。详情参考：https://dev.vivo.com.cn/documentCenter/doc/362#w2-98542835。
	//  - 0：表示正式推送；
	//  - 1：表示测试推送。
	...
	//  - NORMAL：重要消息。
	//  - HIGH：非常重要消息（仅华为支持）。
	// 说明：华为从 2023.09.15 开始基于《华为消息分类标准》对其云端通知和本地通知进行共同管控推送，开发者通过极光服务发起推送时，请注意此字段传值要符合华为官方「云端通知 importance」取值要求，要和【华为云端通知 category 取值】要求对应：https://developer.huawei.com/consumer/cn/doc/development/HMSCore-Guides/message-classification-0000001149358835#section153801515616。
	//
	// 参考文档：
	...
	// 注意事项：
	//  - 华为需完成【自分类权益申请】：https://docs.jiguang.cn/jpush/client/Android/android_channel_id#%E5%8D%8E%E4%B8%BA%E6%B6%88%E6%81%AF%E5%88%86%E7%B1%BB%E4%BD%BF%E7%94%A8%E6%8C%87%E5%8D%97；
	//  - 华为从 2023.09.15 开始基于《华为消息分类标准》对其云端通知和本地通知进行共同管控推送，开发者通过极光服务发起推送时，请注意此字段传值要符合华为官方【华为云端通知 category 取值】要求：https://developer.huawei.com/consumer/cn/doc/development/HMSCore-Guides/message-classification-0000001149358835#section153801515616；
	//  - vivo 具体规则参考【vivo 官方说明】：https://dev.vivo.com.cn/documentCenter/doc/359#w1-36109489；
	//  - OPPO 于 2024.11.20 实施消息分类新规，具体规则参考【OPPO 官方说明】：https://open.oppomobile.com/new/developmentDoc/info?id=13189。
	...
	//  - 仅首次给应用推送【服务与通讯消息】（https://developer.huawei.com/consumer/cn/doc/HMSCore-Guides/message-classification-0000001149358835#section5101818813）时设置有效，
	//  需要配合 DefaultSound 一起使用，详情参考【如何实现自定义铃声】：https://docs.jiguang.cn/jpush/practice/custom_ringtone#%E5%8D%8E%E4%B8%BA%E9%80%9A%E9%81%93%E9%80%9A%E7%9F%A5%E5%AE%9E%E7%8E%B0。
	Sound string `json:"sound,omitempty"`
	// 【可选】华为默认铃声控制开关。
	//
	// 华为官方说明，首次给应用推送【服务与通讯消息】（https://developer.huawei.com/consumer/cn/doc/HMSCore-Guides/message-classification-0000001149358835#section5101818813）时携带 Sound 字段且 DefaultSound 值设置为 false。
	//
	...
	//  - NORMAL：重要消息。
	// 设置为 HIGH 需要向华为申请特殊权限，参考：https://developer.huawei.com/consumer/cn/doc/development/HMSCore-Guides/faq-0000001050042183#section037425218509。
	Urgency string `json:"urgency,omitempty"`
	// 【可选】华为回执 ID。
	//  - 仅华为通道有效。
	//  - 输入一个唯一的回执 ID 指定本次下行消息的回执地址及配置，该回执 ID 可以在【华为回执参数配置】中查看：https://developer.huawei.com/consumer/cn/doc/development/HMSCore-Guides/msg-receipt-guide-0000001050040176#ZH-CN_TOPIC_0000001087208860__li15263162510251。
	ReceiptID string `json:"receipt_id,omitempty"`
	// 【可选】vivo 回执 ID。
	//  - 仅 vivo 通道有效。
	//  - 输入一个唯一的回执 ID 指定本次下行消息的回执地址及配置，该回执 ID 可以在【vivo 回执参数配置】中查看：https://dev.vivo.com.cn/documentCenter/doc/681#w2-33657032。
	CallbackID string `json:"callback_id,omitempty"`
	// 【可选】华为消息类型，仅华为通道有效。
	//  - 0：普通消息（默认值）；
	//  - 1：测试消息。
	// 每个应用每日可发送测试消息 500 条且不受【每日单设备推送数量上限要求】限制：https://developer.huawei.com/consumer/cn/doc/development/HMSCore-Guides/message-restriction-description-0000001361648361#section104849311415。
	TargetUserType *int `json:"target_user_type,omitempty"`
	// 【可选】厂商消息大图标样式。
	//  - 支持 华为 / 荣耀 / OPPO 厂商，使用详情参见【设置图标文档】：https://docs.jiguang.cn/jpush/practice/set_icon#android%E3%80%82；
	//  - 优先使用厂商字段，厂商字段没有填充，则使用 Android 里面定义 LargeIcon 字段：https://docs.jiguang.cn/jpush/server/push/rest_api_v3_push#android；
	...
	// 【可选】厂商消息小图标样式。
	//  - 目前支持 华为 / 荣耀 厂商，使用详情参见【设置图标文档】：https://docs.jiguang.cn/jpush/practice/set_icon#android%E3%80%82；
	//  - 优先使用厂商字段，厂商字段没有填充，则使用 Android 内的 SmallIcon 字段。
	...
	//
	// 此处直接使用第三方审核结果的返回值原数据填充即可，开发者无需关心各个厂商原始协议，对应推必安信息审核 API 响应内容，详见：https://tuibian.mobileservice.cn/。
