...
//
// [developer.apple.com]: https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Payload-key-reference
type IOS struct {
	// 【必填】通知内容。
	...
	//  - 告警通知：JSON Object，支持官方定义的 payload 结构（alert.IosSound），详见 [developer.apple.com] 文档说明；
	//  - 自定义铃声说明：格式必须是 Linear PCM、MA4（IMA/ADPCM）、alaw，μLaw 的一种，将声频文件放到项目 bundle 目录中，且时长要求 30s 以下，否则就是系统默认的铃声，详见 [自定义铃声] 文档说明。
	//
	...
	// 【可选】推送唤醒。
	//  - 推送的时候携带 "content-available":true，说明是 Background Remote Notification，如果不携带此字段则是普通的 Remote Notification，详情参考 [Background Remote Notification]。
	...
	// 【可选】通知优先级和投递时间的中断级别。
	//  - iOS 15 的通知级别，取值只能是 alert.IosInterruptionLevelActive、alert.IosInterruptionLevelCritical、alert.IosInterruptionLevelPassive、alert.IosInterruptionLevelTimeSensitive 中的一个，详情参考 [UNNotificationInterruptionLevel]。
